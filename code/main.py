...
from utils import root_mean_squared_error_numpy, load_dataset_signal_addr, load_dataset_feature_addr, \
    parse_valid_data_all, save_test_result
...
net_params = {'epochs': args.epochs, 'batch_size': args.batch_size, 'early_stopping': args.early_stopping,
              'saved_ckpt_flag': args.saved_ckpt}
...
class experiments():
        ...
        for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
            for session_num in range(1, config[self.dataset_name]['Session_No'] + 1):
                ...
                train_embed, test_embed = spatial_features(config, self.dataset_name, args.riemannian_dist,
                                                           config[self.dataset_name]['params']['Rank_No']).embedding(
                    X_train_signal, X_test_signal)

                Y_pred = spatial_temporal_info_stream(train_embed, test_embed, X_train_features, X_test_features,
                                                      Y_train, Y_test, self.dataset_name, net_params)
    ...
    def run_seed_vig(self):
        ...
        for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
            ...
            for train_index, test_index in kfold_test.split(X_signal):
                ...
                X_train_signal, X_test_signal, X_train_features, X_test_features, Y_train, Y_test = X_signal[
                                                                                                        train_index], \
                                                                                                    X_signal[
                                                                                                        test_index], \
                                                                                                    X_features[
            ...
            save_test_result(self.dataset_name, rmse_value, corcoeff_value)
    ...
    def run_bci(self):
        ...
        for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
            ...
            Y_pred = spatial_temporal_info_stream(train_embed, test_embed, X_train_features, X_test_features, Y_train,
                                                  Y_test, self.dataset_name, net_params)
