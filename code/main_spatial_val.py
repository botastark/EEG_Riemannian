...
class experiments:
        ...
        for rank_num in tqdm(range(1, config[self.dataset_name]['Channel_No'])):
            ...
            for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
                for session_num in range(1, config[self.dataset_name]['Session_No'] + 1):
                    ...
                    ####################################################
                    train_embed, test_embed = spatial_features(config, self.dataset_name, args.riemannian_dist,
                                                               rank_num).embedding(X_train, X_test)
                    ...
                    for train_index, test_index in kfold.split(train_embed):
                        ...
                        Y_pred = spatial_info_stream(X_train, X_val, X_test, Y_train, Y_val, Y_test, self.dataset_name,
                                                     net_params)
        ...
        for rank_num in tqdm(range(1, config[self.dataset_name]['Channel_No'])):
            ...
            for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
                ...
                for train_index, test_index in kfold_test.split(X):
                    ...
                    rmse_value = root_mean_squared_error_numpy(Y_val_total,
                                                               Y_pred_total)  # RMSE value for all 885 samples
            ...
            save_spatial_val_result(self.dataset_name, rmse_array, corr_array, rank_num)
    ...
    def run_bci(self):
        ...
        data_train_addr, data_test_addr, label_train_addr, label_test_addr = list(addr_dict.values())
        # print(data_train_addr)
        # print(data_test_addr)
        # print(label_train_addr)
        # print(label_test_addr)
        ...
        for rank_num in tqdm(range(1, config[self.dataset_name]['Channel_No'] + 1)):
            ...
            for subject_num in track(range(1, config[self.dataset_name]['Subject_No'] + 1)):
                # ____________________LOAD DATA____________________#
                # print("Loading data")
                X_train = np.load(data_train_addr.format(subject_num), allow_pickle=True)  # trials, frequency_band, channel, signal
                Y_train = np.load(label_train_addr.format(subject_num), allow_pickle=True)
                X_test = np.load(data_test_addr.format(subject_num), allow_pickle=True)
                Y_test = np.load(label_test_addr.format(subject_num), allow_pickle=True)
...
if __name__ == '__main__':
    ...
    tf.random.set_seed(args.cpu_seed)
