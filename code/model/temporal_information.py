...
def temporal_info_stream(X_train, X_val, X_test, Y_train, Y_val, Y_test, dataset, net_params):
    ...
    X = np.transpose(X, (0, 2, 3, 1))  # trial, timestep, channel, features
    ...
    for layer_num in range(1, net_params['layer_num'] + 1):
        if not net_params['bidirectional_flag']:
            if layer_num == 1:
    ...
    if dataset == 'SEED' or dataset == 'BCI_IV_2a':
        model.compile(loss=tf.keras.losses.m(),
    ...
    '''
    callbacks = [
        tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='auto', patience=net_params['early_stopping']),
        tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path, monitor='val_loss', mode='auto',
                                           save_weights_only=True, save_best_only=True)]

    history = model.fit(X_train, Y_train, epochs=net_params['epochs'], batch_size=net_params['batch_size'], verbose=1,
                        shuffle=True,
