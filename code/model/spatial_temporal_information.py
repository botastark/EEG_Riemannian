...
def corcoeff(y_true, y_pred):
    ...
    den = tf.square(tf.reduce_sum(tf.square(y_true - tf.reduce_mean(y_true))) * tf.reduce_sum(
        tf.square(y_pred - tf.reduce_mean(y_pred))))
    return num / den


def spatial_temporal_info_stream(train_embed, test_embed, X_train_features, X_test_features, Y_train, Y_test, dataset,
                                 net_params):
    '''
    Spatal Info Stream
    '''
        ...
        if config[dataset]['params']['Bi_LSTM'] == False:
            ...
            if layer_num == 1:
                lstm_out = tf.keras.layers.Bidirectional(
                    tf.keras.layers.LSTM(units=256, return_sequences=True, activation="tanh",
                                         recurrent_activation="sigmoid", use_bias=True,
                                         kernel_initializer="glorot_uniform",
                                         recurrent_initializer="orthogonal",
                                         bias_initializer="zeros"))(inputs_temporal)
    ...
    output = tf.keras.layers.Dense(config[dataset]['output_unit'], activation=config[dataset]['act_func'])(
        attention_score_final)
    ...
    if net_params['saved_ckpt_flag'] == True:
        ...
        '''
        callbacks = [
            tf.keras.callbacks.EarlyStopping(monitor='loss', mode='auto', patience=net_params['early_stopping']),
            tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path, monitor='loss', mode='auto',
                                               save_weights_only=True, save_best_only=True)]

        history = model.fit([train_embed, X_train_features], Y_train, epochs=net_params['epochs'],
                            batch_size=net_params['batch_size'], verbose=1, shuffle=True,
